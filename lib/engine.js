...
import { buildNode, getOpenTag, Path, buildPropertyTag } from '@bablr/agast-helpers/path';
...
export class SpamexEngine extends PatternEngine {
  ...
  feed(tag) {
    ...
    if (path && parseTag(getOpenTag(path.node)).value.selfClosing) {
      path = path.parent;
    }
    ...
    switch (tag.type) {
      ...
      case GapTag: {
        ...
        path.parent.node = Path.from(parentNode).advance(
          buildPropertyTag(BList.fromValues([printTag(this.ref, porcelain), '', '', '', node], 1)),
        ).node;
        break;
      }
      ...
      case OpenNodeTag: {
        ...
        const node = buildNode(Tags.fromValues([printTag(tag, porcelain)], 1));
        ...
        if (parentPath) {
          ...
          parentPath.node = Path.from(parentNode)
            .advance(printTag(this.ref, porcelain))
            .advance(node).node;
        } else {
          this.rootPath = path;
        }
    ...
    switch (tag.type) {
      ...
      case GapTag:
      case CloseNodeTag: {
        if (tag.type === CloseNodeTag && !path.depth) break;
