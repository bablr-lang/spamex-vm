...
import { buildProperty, parseTag } from '@bablr/agast-helpers/builders';
    ...
    switch (tag.type) {
      ...
      case GapTag: {
        ...
        path.parent.node = Path.from(parentNode).advance(
          buildProperty(BList.fromValues([printTag(this.ref), BList.empty(), node], 1)),
        ).node;
        break;
      }
      ...
      case OpenNodeTag: {
        ...
        const node = buildNode(Tags.fromValues([Tags.empty(), printTag(tag)], 1));
        ...
        if (parentPath) {
          ...
          parentPath.node = Path.from(parentNode).advance(printTag(this.ref)).advance(node).node;
        } else {
          this.rootPath = path;
        }
