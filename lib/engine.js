...
import { createNode, finalizeNode, Path } from '@bablr/agast-helpers/path';
  ...
  feed(tag) {
    ...
    switch (tag.type) {
      ...
      case OpenNodeTag: {
        ...
        const node = { flags, type, tags: [], attributes };
        ...
        if (parentPath) {
          ...
          if (!parentNode.tags.length) {
            throw new Error('Nodes must follow references');
          }

          Path.from(parentNode).advance(this.ref).advance(buildNodeTag(node));
        } else {
          this.rootPath = path;
        }
