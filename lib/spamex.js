import { getCooked, getRoot, printType } from '@bablr/agast-helpers/tree';
...
const nodeMatcher = (matcher, qIdx) => {
  return compose(
    openNodeTagMatcher(matcher.properties.open.node),
...
const visitors = {
  ...
  PropertyMatcher: (node, state, visit) => {
    return visit(node.properties.nodeMatcher.node);
  },
