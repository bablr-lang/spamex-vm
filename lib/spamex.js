...
const openNodeTagMatcher = (matcher) => {
  ...
  const matcherName = matcher_.name;
  const matcherType = matcher_.type;
  return (next) => {
    return {
      type: sym.cont,
      width: 1,
      ...
      match: (state, { path, value: token }) => {
        ...
        const name = token.value.name.description;

        const matched = matcherType ? matcherType === '?' && name : matcherName === name;
...
export const buildPatternInternal = (frag, options = freeze({})) => {
  const pState = {
