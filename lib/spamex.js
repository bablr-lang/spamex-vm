import { get, getRoot, printType } from '@bablr/agast-helpers/tree';
...
const nodeMatcher = (matcher, qIdx) => {
  return compose(
    openNodeTagMatcher(get('open', matcher)),
    repeat(anyMatcher, qIdx, false),
    closeNodeTagMatcher(),
  );
};

const openNodeTagMatcher = (matcher) => {
  const matcherType = reifyExpression(matcher).type;
  return (next) => {
    return {
      type: sym.cont,
      width: 1,
      ...
      match: (state, { path, value: token }) => {
        if (token.type !== OpenNodeTag || !token.value.type) return null;

        const type = token.value.type.description;

        const matched = (matcherType === '?' && type && type !== sym.gap) || matcherType === type;
...
const visitors = {
  Alternative: (node, state, visit) => {
    ...
    return elements.map((element) => visit(element.node)).reduce(compose, identity);
  },

  Group: (node, state, visit) => {
    const alternatives = get('alternatives', node);
  ...
  PropertyMatcher: (node, state, visit) => {
    return visit(get(['valueMatcher', 'nodeMatcher'], node));
  },

  TreeNodeMatcher: (node, state) => {
...
const buildPattern = (matcher, state) => {
  const qIdxs = (state.qIdxs = []);
  const reset = resetRepetitionStates(qIdxs, state.initialRepetitionStates);
  const qIdx = ++state.qIdx;
  state.qIdxs.push(qIdx);
...
export const buildPatternInternal = (frag, options = {}) => {
  const pState = {
  ...
  const node = getRoot(getEmbeddedMatcher(frag));

  const seq = buildPattern(visit(node, pState, visitors), pState);
